...
if cached_data:
    ...
    with t_vip:
        # 💡 變更點 1：調整欄位比例從 [1, 2] 改為 [1.3, 2]，讓圖表往右移
                    ...
                    if sel:
                        ...
                        if row_list: 
                            try:
                                ...
                                # 💡 變更點 2：增加精簡月營收报告區塊，顯示兩列（Q1~Q2）
                                st.markdown("---")
                                st.markdown("#### 📅 精簡月營收報告（Q1~Q2）")
                                q1_m = row.get("_known_q1_months", [0,0,0])
                                q2_m = row.get("_known_q2_months", [0,0,0])
                                
                                # 第一列：1, 2, 3月
                                r1c1, r1c2, r1c3 = st.columns(3)
                                with r1c1: st.markdown(f'<p class="div-month-label">1月營收</p><p class="div-month-value">{q1_m[0]:.2f} 億</p>', unsafe_allow_html=True)
                                with r1c2: st.markdown(f'<p class="div-month-label">2月營收</p><p class="div-month-value">{q1_m[1]:.2f} 億</p>', unsafe_allow_html=True)
                                with r1c3: st.markdown(f'<p class="div-month-label">3月營收</p><p class="div-month-value">{q1_m[2]:.2f} 億</p>', unsafe_allow_html=True)
                                
                                # 第二列：4, 5, 6月
                                r2c1, r2c2, r2c3 = st.columns(3)
                                with r2c1: st.markdown(f'<p class="div-month-label">4月營收</p><p class="div-month-value">{q2_m[0]:.2f} 億</p>', unsafe_allow_html=True)
                                with r2c2: st.markdown(f'<p class="div-month-label">5月營收</p><p class="div-month-value">{q2_m[1]:.2f} 億</p>', unsafe_allow_html=True)
                                with r2c3: st.markdown(f'<p class="div-month-label">6月營收</p><p class="div-month-value">{q2_m[2]:.2f} 億</p>', unsafe_allow_html=True)
                ...
                with c2:
                    if sel and row_list: 
                        try:
                            ...
                            # 💡 變更點 3：優化 Altair 圖表，增加 Tooltip 並提高觸摸/滑鼠懸停靈敏度
                            # 建立一個基礎圖表
                            ...
                            # 建立一個透明的「選擇層」，專門用來捕捉觸摸/滑鼠事件，提高靈敏度
