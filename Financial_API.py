# ==========================================
# 📂 檔案名稱： Financial_API.py (終極防撞防當機版 - 業外 100% 直讀校準版)
# 💡 更新內容： 徹底刪除自作聰明的業外計算公式，直接抓取表單精準欄位！
# ==========================================
...
def auto_strategic_model(name, current_month, rev_last_10, rev_last_11, rev_last_12, rev_this_1, rev_this_2, rev_this_3, base_q_eps, non_op_ratio, base_q_total_rev, ly_q1_rev, ly_q2_rev, ly_q3_rev, ly_q4_rev, y1_q1_rev, y1_q2_rev, y1_q3_rev, y1_q4_rev, recent_payout_ratio, current_price, contract_liab, contract_liab_qoq, acc_eps, declared_div, actual_q1_eps):
    ...
    # 🌟 EPS 黃金公式：近期EPS * (1-業外%) * (動態預估營收 / 基準營收)
    ...
    try:
        ...
        def parse_df(df):
            ...
            for idx, row in df.iterrows():
                ...
                # 🌟 核心修正：徹底刪除舊的瞎算公式，100% 只讀取您精準表單的業外佔比！
                non_op_ratio = v(get_col("單季業外損益佔稅前淨利") or get_col("業外損益佔稅前淨利") or get_col("業外", "稅前淨利") or get_col("業外佔比"))
                ...
                new_entry = {
                    ...
                    "base_q_total_rev": rev_q4 if eps_q4 != 0 else rev_q3, # 用Q4營收作為EPS精準分母
                            ...
                            for i, q in enumerate(["Q1", "Q2", "Q3", "Q4"]):
                                def clean_val_list(lst, idx):
                                    try:
                                        ...
                                        v = lst[idx]
                                        fv = float(v)
                                        return fv if not math.isnan(fv) and
