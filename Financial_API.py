        ...
        for ws in worksheets:
            if "當年度表" in ws.title:
                ...
                if data and len(data) > 1:
                    gen_current_dfs.append(pd.DataFrame(data[1:], columns=data[0]))
            elif "歷史表單" in ws.title:
                data = ws.get_all_values()
                if data and len(data) > 1:
                    gen_history_dfs.append(pd.DataFrame(data[1:], columns=data[0]))
            elif "金融股" in ws.title:
                data = ws.get_all_values()
                if data and len(data) > 1:
                    fin_dfs.append(pd.DataFrame(data[1:], columns=data[0]))
                    
        df_current = pd.concat(gen_current_dfs, ignore_index=True) if gen_current_dfs else pd.DataFrame()
