...
# 💡 更新內容： 
#    1. 網頁開啟自動運算 + Slider連動更新
#    2. Q2(4,5月) 黃金動態推估與圖表(已公布)完美顯示
#    3. 側邊欄 CSS 修復，避免按鈕消失
# ==========================================
...
def auto_strategic_model(name, current_month, rev_last_10, rev_last_11, rev_last_12, rev_this_1, rev_this_2, rev_this_3, rev_this_4, rev_this_5, base_q_eps, non_op_ratio, base_q_total_rev, ly_q1_rev, ly_q2_rev, ly_q3_rev, ly_q4_rev, y1_q1_rev, y1_q2_rev, y1_q3_rev, y1_q4_rev, recent_payout_ratio, current_price, contract_liab, contract_liab_qoq, acc_eps, declared_div, actual_q1_eps):
    ...
    # ==========================================
    # 🌟 Q2 黃金防守預估 (4月、5月邏輯) & 圖表實收累計
    # ==========================================
    if current_month <= 3:
        actual_known_q2 = 0
        dynamic_est_q2_rev = dynamic_est_q1_rev
    elif current_month == 4:
        actual_known_q2 = rev_this_4
        if rev_this_4 > 0:
        ...
        actual_known_q2 = rev_this_4 + rev_this_5
    ...
    return {
        ...
        "_known_qs": [round(actual_known_q1, 2), round(actual_known_q2, 2), 0, 0], # 💡 將實際收到的 Q2 資料餵給圖表
        ...
        def parse_df(df):
            ...
            for idx, row in df.iterrows():
                ...
                new_entry = {
                    ...
                    "rev_this_4": v(get_col(f"{this_y}M04", "營收", ex=["增", "率", "%"])), # 💡 抓取4月
                    "rev_this_5": v(get_col(f"{this_y}M05", "營收", ex=["增", "率", "%"])), # 💡 抓取5月
                    "base_q_eps": base_eps, 
...
if 'last_simulated_month' not in st.session_state:
    st.session_state['last_simulated_month'] = current_real_month
